...
__declspec(naked) NTSTATUS fWjqYDbwxkz(
...
__declspec(naked) NTSTATUS FEOpwBxgCn(
	IN HANDLE ProcessHandle,
...
__declspec(naked) NTSTATUS HzVCOJLhNyaEZsi(
...
__declspec(naked) NTSTATUS jQHNPZrkGGfIPGFs(
...
__declspec(naked) NTSTATUS HXgcBFZuWRhFsMtR(
...
__declspec(naked) NTSTATUS fDnbiaXGZpi(
...
__declspec(naked) NTSTATUS szxfzVXTNWYNH(
	IN HANDLE ProcessHandle,
	IN PVOID BaseAddress)
{
	asm(
...
__declspec(naked) NTSTATUS GAQFpVUodIxpYP(
	IN HANDLE Handle)
{
	asm(
		"mov [rsp +8], rcx \n"
...
__declspec(naked) NTSTATUS HkfVkMdIVNxC(
	IN HANDLE ProcessHandle,
	IN OUT PVOID * BaseAddress,
	IN ULONG ZeroBits,
	IN OUT PSIZE_T RegionSize,
